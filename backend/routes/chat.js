...
chatRoute.post('/', async (req, res) => {
  ...
  try {
    ...
    if (lower.includes('best sellers') || lower.includes('popular products')) {
      ...
      return res.json({ reply: "Sorry, we don't have best sellers to show right now." });
    }

    // Common questions
    if (lower.includes('price')) {
      return res.json({ reply: 'Prices vary depending on the product. Can you tell me which item?' });
    }
    if (lower.includes('delivery')) {
      return res.json({ reply: 'We offer delivery within 3-5 business days.' });
    }
    if (lower.includes('return')) {
      return res.json({ reply: 'You can return items within 7 days of delivery.' });
    }
    ...
    if (lower.includes('shipping cost') || lower.includes('shipping fee')) {
      return res.json({ reply: 'Shipping costs depend on your location and order size. Free shipping on orders above Rs.3000!' });
    }
    if (lower.includes('order status') || lower.includes('track order')) {
      return res.json({ reply: 'You can track your order status in your account dashboard or provide your order ID here.' });
    }
    if (lower.includes('cancel order') || lower.includes('return order')) {
      return res.json({ reply: 'Orders can be cancelled within 24 hours of purchase. Returns accepted within 7 days of delivery.' });
    }
    if (lower.includes('discount') || lower.includes('offer') || lower.includes('sale')) {
      return res.json({ reply: 'We regularly offer discounts and sales. Follow our newsletter or website for the latest offers!' });
    }
    if (lower.includes('store location') || lower.includes('address')) {
      return res.json({ reply: 'We are located at Bhaktapur, and also available online 24/7!' });
    }
    if (lower.includes('contact') || lower.includes('support')) {
      return res.json({ reply: 'You can reach our support team at support@traditionshop.com or call +977-984-6543210.' });
    }
    if (lower.includes('product availability') || lower.includes('in stock')) {
      return res.json({ reply: 'You can ask about any product by name to check its availability.' });
    }
    if (lower.includes('what product') || lower.includes('show product') || lower.includes('products available')) {
      return res.json({ reply: 'We have sarees, jewelry, topis, and more! Ask about any item by name.' });
    }
