...
const Collection = () => {
    ...
    const applyFilter = () => {
        ...
        if (subCategory.length > 0) {
            productsCopy = productsCopy.filter((item) => subCategory.includes(item.subCategory)); // Adjusted to match database field
        }
                        ...
                        <div className={`border border-gray-300 pl-5 py-4 mt-6 ${showFilter ? '' : 'hidden'} sm:block`}>
                            ...
                            {loading ? (
                                ...
                                <div className="flex flex-col gap-4 text-sm font-light text-gray-700">
                                    {categories.length > 0 ? (
                                        categories.map((cat) => (
                                            <label key={cat._id} className="flex gap-2 items-center cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    className="w-4 h-4"
