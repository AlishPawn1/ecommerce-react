...
const ProfileCard = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.number || !/^(9[876]\d{8})$/.test(formData.number)) {
            errors.number = 'Phone number must be 10 digits starting with 98, 97, or 96';
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-white to-pink-100 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
                <div className="flex flex-col items-center">
                    ...
                    <h1 className="text-3xl font-extrabold text-gray-900 mt-4">{user.name}</h1>
                    <p className="text-gray-500 text-sm">{user.email}</p>
                    <p className="text-gray-600 text-center mt-3">{user.isVerified ? 'Verified' : 'Not Verified'}</p>
                </div>
